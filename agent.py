...
class Agent_subgrad(Agent_opt):
    def update(self,k):
        ...
        d_bf = self.d_i()
        # print(self.weight, self.z_j)
        self.x_i = self.x_i + sum - self.step(k) * d_bf
        # self.s_update(d_bf)
        self.trigger_check(k)
...
all_agent_subgrad = [Agent_subgrad(n, i, weight[i], 0.05) for i in range(n)]
for k in range(200):
    for i in range(n):
        for j in range(n):
            all_agent[i].receive(all_agent[j].send(i), j)
            all_agent_opt[i].receive(all_agent_opt[j].send(i), j)
            all_agent_subgrad[i].receive(all_agent_subgrad[j].send(i), j)

    for i in range(n):
        all_agent[i].update()
        all_agent_opt[i].update(k)
        all_agent_subgrad[i].update(k)
    for i in range(n):
        print(all_agent[i].z_i, all_agent_opt[i].x_i,all_agent_subgrad[i].x_i)
